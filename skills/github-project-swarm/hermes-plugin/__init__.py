...
def _setup_cli(parser):
    ...
    build_parser({name:None for name in "init status reconcile pause resume doctor validate explain retire merge-policy prepare activate disable".split()},parser)
