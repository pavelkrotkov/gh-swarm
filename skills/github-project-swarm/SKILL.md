...
Recovery is artifact-first and bounded per reconcile: implementation uses git/PR evidence; reviewer/adjudicator recovery uses exact-head GitHub publications and idempotent Kanban keys. Failed attempts keep their immutable keys; later reconciles advance to the next `:aN` key, one fresh attempt after the configured initial bound, so transient provider failures can recover without operator state edits. A newly created ACTIVE task may remain `RUNNING` without a worker run for up to the persisted 300-second startup grace; after that it fails closed, and missing/future creation timestamps fail closed immediately.
