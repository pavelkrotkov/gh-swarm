...
def request_exact_head_merge(config,issue_number,expected_head,reader=None,merger=None):
    ...
    actual_merger.merge(config.repo,pr.number,head); confirmed=_confirm_after_request(config,issue_number,head,actual_reader,actual_merger); return confirmed or MergeResult(MergeResultState.REQUESTED,pr.number,head)
