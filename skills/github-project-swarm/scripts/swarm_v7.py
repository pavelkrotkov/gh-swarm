...
def _merge(obs,config,reason):
    fixed={MergeGate.BLOCKED:(Phase.MERGE_BLOCKED,None,"merge authorization is blocked by current GitHub gates"),MergeGate.UNKNOWN:(_STALL,None,"merge gate state is unknown")}.get(obs.merge_gate); ready={"manual":(Phase.AWAITING_MANUAL_MERGE,None,f"manual merge policy; {reason}"),"automatic":(Phase.READY_TO_MERGE,Action.MERGE,reason)}.get(config.merge_policy,(_STALL,None,"merge policy is unknown")); return fixed if fixed else ready
def _unsafe(obs):
