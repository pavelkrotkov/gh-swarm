...
def dispatch_attempts(runtime,adapter,key,spec):
    _need(runtime.max_attempts>0,"max_execution_attempts must be positive"); cursor=runtime.cursors.get(key); used=int(cursor.get("attempt") or 0) if isinstance(cursor,dict) else 0; limit=max(runtime.max_attempts,used+1)
...
def _start_review(ctx,adjudicate):
    pr,target=_target(ctx); rows=() if pr.adjudication is None else (pr.adjudication,); used=max((int(row.get("attempt") or 0) for row in ctx.runtime.cursors.values() if isinstance(row,dict)),default=0); attempts=max(ctx.runtime.max_attempts,used+1); results=(reconcile_adjudication(ctx.runtime.config,target,pr.reviewers,rows,ctx.kanban,attempts),) if adjudicate else reconcile_reviewers(ctx.runtime.config,target,pr.reviewers,ctx.kanban,attempts); return _remember(ctx.runtime,results)
