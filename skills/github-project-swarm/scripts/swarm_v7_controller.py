...
from collections import namedtuple; from dataclasses import dataclass, field, replace; from pathlib import Path; import time; from swarm_v7 import Action, AdjudicationDecision, DependencyState, ExecutionState, ManifestV7, Phase, ReviewState, plan_issue; from swarm_v7_workspace import GitWorkspace, WorkspaceSpec, branch_name, worktree_path; from swarm_v7_github import GhReader, observe_issue as observe_github; from swarm_v7_kanban import KanbanAdapter, Outcome, TaskSpec, semantic_key, worker_body; from swarm_v7_merge import GhMerger, request_exact_head_merge; from swarm_v7_review import ExactHeadTarget, _model, reconcile_adjudication, reconcile_reviewers
...
def observation_payload(observed,config): pr=observed.github.pull_request; planner=observed.planner; pull=None if pr is None else {"number":pr.number,"url":pr.url,"head":pr.head,"base":pr.base,"state":pr.state,"draft":pr.draft,"merged_at":pr.merged_at,"merge_sha":getattr(pr,"merge_sha",None)}; raw=() if pr is not None and pr.merged_at else (*_config_gates(config,pr),*_planner_gates(observed,config),*_shape_gates(config,pr,planner),*_mergeability_gates(pr)); gates=[{"code":code,"detail":str(detail)} for code,detail in raw if code and detail]; return {"issue_state":observed.github.issue_state,"blockers":[{"issue":row.issue_number,"state":row.state,"internal":row.internal,"merged_at":row.merged_at} for row in observed.github.blockers],"pr":pull,"execution":dict(observed.execution),"base_current":planner.base_current,"unsafe_reason":planner.unsafe_reason,"gates":gates}
