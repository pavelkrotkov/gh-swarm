...
def _slot(runtime,key,kanban,execution):
    ...
    attempt=int(cursor.get("attempt",1)); facts=kanban.observe(str(cursor["task_id"])); execution[key]=f"{facts.status}:a{attempt}"
    ...
    if facts.outcome is not Outcome.FAILURE: return (ExecutionState.RUNNING,None) if facts.outcome is Outcome.ACTIVE else (ExecutionState.IDLE,None)
