...
def _issue_task(row,swarm,issue): title=str(row.get("title") or ""); number=title.partition("] #")[2].split(" ",1)[0]; lines=str(row.get("body") or "").splitlines(); worker=title.startswith(("[implement]","[revise]")) and len(lines)>2 and lines[2]==f"Expected branch: swarm/{swarm}/{issue}"; review=title.startswith("[review ") and len(lines)>2 and lines[2].startswith(f"Required publication marker: <!-- hermes-swarm-review:{swarm}:{issue}:"); adjudicate=title.startswith("[adjudicate ") and len(lines)>1 and lines[1].startswith(f"Required adjudication marker: <!-- hermes-swarm-adjudication:{swarm}:{issue}:"); return _STATUS.get(str(row.get("status") or "").strip().lower()) is Outcome.ACTIVE and number==str(issue) and any((worker,review,adjudicate))
