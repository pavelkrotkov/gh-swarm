...
class KanbanAdapter:
    ...
    def block_issue(self,issue,reason): ids=tuple(task for raw in json.loads(self._run(("list",)) or "[]") for row,task in (_task(raw),) if (match:=_TASK_TITLE.match(str(row.get("title") or ""))) and _STATUS.get(str(row.get("status") or "").strip().lower()) is Outcome.ACTIVE and match.group(1)==str(issue)); ids and self.runner(("hermes","kanban","--board",self.board,"block",ids[0],reason)+(( "--ids",*ids[1:]) if len(ids)>1 else ()),self.cwd,self.timeout_s); return ids
