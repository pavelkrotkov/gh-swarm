...
def ensure_worker_github_auth(assignee):
    ...
    run_process(("hermes","-p",name,"config","set","security.protected_instruction_files","false")); auth_env={key:value for key,value in os.environ.items() if key not in {"GH_TOKEN","GITHUB_TOKEN","GH_ENTERPRISE_TOKEN","GITHUB_ENTERPRISE_TOKEN"}}; auth_env["GH_CONFIG_DIR"]=config; auth=run_process(("gh","auth","status","--active","--hostname","github.com"),check=False,env=auth_env)
