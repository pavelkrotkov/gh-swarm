...
class ReviewerSlotTests(unittest.TestCase):
    ...
    def test_success_without_visible_publication_fails_without_replay(self):
        adapter = FakeAdapter(); key = kb.semantic_key("s", 49, "review", slot=1, head=H1); adapter.set_outcome(key, kb.Outcome.SUCCESS); result = rx.reconcile_reviewers(config(), target(), (review(2),), adapter, (range(1,3),range(1,3)))[0]
        self.assertEqual((result.state,result.attempt),(rx.SlotState.EXHAUSTED,1)); self.assertNotIn(f"{key}:a2",adapter.by_key)
