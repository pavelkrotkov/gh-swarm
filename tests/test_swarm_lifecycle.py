...
class LifecycleTests(unittest.TestCase):
    ...
    def test_successful_review_at_attempt_bound_without_publication_stalls(self):
        rt=runtime(issues=(1,)); head="1"*40; key=semantic_key(rt.config.swarm_id,1,"review",slot=1,head=head); rt.cursors[key]={"task_id":"task-r2","attempt":2}; planner=Observation(1,pr_head=head,ci=CiState.PASSED); github=SimpleNamespace(issue_number=1,pull_request=SimpleNamespace(head=head,reviewers=(SimpleNamespace(slot=2),)),planner=planner); adapter=Mock(); adapter.observe.return_value=SimpleNamespace(outcome=Outcome.SUCCESS,status="done",has_run=True,task_id="task-r2")
        observed=_overlay(rt,github,adapter,{}); plan=plan_issue(observed,rt.config); self.assertEqual((observed.review,plan.phase,plan.action),(ReviewState.UNKNOWN,Phase.EXECUTION_STALLED,None)); self.assertIn("without durable publication",observed.unsafe_reason)
    def test_exhausted_review_cursor_extends_only_its_slot(self):
        ...
        self.assertEqual(reconcile.call_args.args[-1],(range(2,4),range(1,3)))
    def test_successful_adjudication_at_bound_does_not_extend(self):
        rt=runtime(issues=(1,)); head="1"*40; key=semantic_key(rt.config.swarm_id,1,"adjudication",head=head); rt.cursors[key]={"task_id":"task-a2","attempt":2}; pr=SimpleNamespace(number=3,head=head,reviewers=(SimpleNamespace(slot=1),SimpleNamespace(slot=2)),adjudication=None); obs=Observation(1,pr_head=head,ci=CiState.PASSED,review=ReviewState.DISPUTED); result=SimpleNamespace(task_id=None,attempt=None,semantic_key=key,state=SimpleNamespace(value="EXHAUSTED")); item=PlannedIssue(IssueObservation(SimpleNamespace(issue_number=1,pull_request=pr),obs,{key:"done:a2"}),Plan(Phase.NEEDS_ADJUDICATION,Action.START_ADJUDICATION,"retry",head,"intent"))
        with patch("swarm_v7_controller.reconcile_adjudication",return_value=result) as reconcile: apply_plan(rt,item,reader=Mock(),kanban=Mock(),workspace=Mock(),merger=Mock())
        self.assertEqual(reconcile.call_args.args[-1],range(2,3))
    def test_exhausted_implementation_attempt_resumes_at_persisted_identity(self):
        rt=runtime(issues=(1,)); key="swarm:demo:issue:1:implementation"; rt.cursors[key]={"task_id":"task-100","attempt":100}; adapter=Mock(); adapter.create.side_effect=("task-100","task-101"); failure=SimpleNamespace(outcome=Outcome.FAILURE,status="blocked",has_run=True,task_id="task-100"); active=SimpleNamespace(outcome=Outcome.ACTIVE,status="running",has_run=True,task_id="task-101"); adapter.observe.side_effect=(failure,failure,active); self.assertEqual(_slot(rt,key,adapter,{}),(ExecutionState.IDLE,None)); result=dispatch_attempts(rt,adapter,key,Mock())
        self.assertEqual(result.task_ids,("task-101",)); self.assertEqual([call.args[2] for call in adapter.create.call_args_list],[100,101]); self.assertEqual(rt.cursors[key]["attempt"],101)
    def test_slot_maps_active_success_and_retryable_failure(self):
        ...
        for attempt,outcome,status,expected in ((1,Outcome.ACTIVE,"running",ExecutionState.RUNNING),(1,Outcome.SUCCESS,"done",ExecutionState.IDLE),(2,Outcome.SUCCESS,"done",ExecutionState.IDLE),(1,Outcome.FAILURE,"blocked",ExecutionState.IDLE),(2,Outcome.FAILURE,"archived",ExecutionState.FAILED)):
            with self.subTest(outcome=outcome,attempt=attempt): rt.cursors[key]["attempt"]=attempt; adapter.observe.return_value=SimpleNamespace(outcome=outcome,status=status,has_run=True,task_id="task-1"); self.assertEqual(_slot(rt,key,adapter,{})[0],expected)
    def test_completed_unpushed_revision_advances_to_next_attempt(self):
        rt=runtime(issues=(1,)); key=semantic_key(rt.config.swarm_id,1,"revision",head="1"*40); rt.cursors[key]={"task_id":"task-1","attempt":1}; adapter=Mock(); failed=SimpleNamespace(outcome=Outcome.FAILURE,status="run_completed",has_run=True,task_id="task-1"); active=SimpleNamespace(outcome=Outcome.ACTIVE,status="running",has_run=True,task_id="task-2"); adapter.observe.side_effect=(failed,failed,active); adapter.create.side_effect=("task-1","task-2")
        self.assertEqual(_slot(rt,key,adapter,{}),(ExecutionState.IDLE,None)); result=dispatch_attempts(rt,adapter,key,Mock()); self.assertEqual((result.outcome,result.task_ids),("active",("task-2",))); self.assertEqual(rt.cursors[key]["attempt"],2)
