...
class MigrationTests(unittest.TestCase):
    def test_preflight_reports_release_capabilities_scheduler_manifest_and_workers(self):
        ...
        self.assertEqual(got["manifest"]["repo"],"owner/repo"); self.assertTrue(all(got["capabilities"].values())); self.assertEqual(got["active_tasks"][0]["id"],"t1"); self.assertTrue(got["release"].endswith("current"))
    ...
    def test_mid_cutover_failure_leaves_swarm_paused_and_never_restores_legacy_timer(self):
        ...
        with patch.object(migration,"preflight",return_value=report()),patch.object(migration,"run",side_effect=fake_run),patch.object(migration,"wait_inactive"),patch.object(migration,"wait_lock"),patch.object(migration,"backup",return_value=Path("/backup")):
