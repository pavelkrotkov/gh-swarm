...
class CliContractTests(unittest.TestCase):
    ...
    def test_retire_persists_reason_and_removes_issue_from_active_scope(self):
        rt=runtime(); adapter=Mock(); adapter.watchdog.return_value={}
        with tempfile.TemporaryDirectory() as td,patch.object(cli,"STATE",Path(td)),redirect_stdout(StringIO()):
            ...
            with patch.object(cli,"KanbanAdapter",return_value=adapter),patch.object(cli,"plan_once") as plan: self.assertEqual(cli.reconcile_runtime(saved),[]); plan.assert_not_called()
        self.assertEqual(saved.config.issues,()); self.assertEqual(saved.retired_issues,{"7":"closed externally"}); self.assertEqual((event["outcome"],event["detail"]),("retired","closed externally"))
