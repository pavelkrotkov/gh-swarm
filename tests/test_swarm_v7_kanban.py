...
class ContractTests(unittest.TestCase):
    ...
    def test_terminal_issue_cleanup_blocks_only_active_swarm_cards_and_is_idempotent(self):
        fake=FakeHermes(); adapter=kb.KanbanAdapter("board",runner=fake); stale=adapter.create(spec(title="[revise] #57",body="You are worker\n\nExpected branch: swarm/s/57\n"),"stale"); review=adapter.create(spec(title="[review abcdef12] #57 reviewer 1",body="Review PR\nReviewer slot: 1\nRequired publication marker: <!-- hermes-swarm-review:s:57:v1:head -->"),"review"); done=adapter.create(spec(title="[adjudicate abcdef12] #57",body="Adjudicate PR\nRequired adjudication marker: <!-- hermes-swarm-adjudication:s:57:head -->"),"done"); collision=adapter.create(spec(title="[revise] #57",body="You are worker\n\nExpected branch: swarm/other/57\n\nExpected branch: swarm/s/57\n"),"collision"); other=adapter.create(spec(title="[revise] #58",body="You are worker\n\nExpected branch: swarm/s/58\n"),"other")
