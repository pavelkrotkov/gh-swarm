...
class ContractTests(unittest.TestCase):
    ...
    def test_stale_or_expired_running_run_is_retryable_failure(self):
        fake=FakeHermes(); adapter=kb.KanbanAdapter("board",runner=fake); task_id=adapter.create(spec(),"semantic"); fake.tasks[task_id]["status"]="running"; old={"id":1,"started_at":100,"ended_at":125,"outcome":"completed","max_runtime_seconds":20}; active={"id":2,"started_at":120,"ended_at":None,"outcome":None,"max_runtime_seconds":20}; fake.tasks[task_id]["runs"]=[active,old]
